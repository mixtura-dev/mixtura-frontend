  ...
  <section class="flex flex-col justify-center items-center pt-32">
    <div class="flex flex-col px-5 max-w-lg w-full">
      <h1 class="lg:text-3xl text-2xl tracking-tight font-bold mb-2">
        {{ $t('form.verifyPasswordReset.title') }}
      </h1>
      <p class="text-muted-foreground mb-6">
        {{ $t('form.verifyPasswordReset.subtitle', { email }) }}
      </p>
...
import { useVerifyResetPasswordMutation } from '@/api/queries/user/useAuthQuery'
...
const onSubmit = handleSubmit((values) => {
  ...
  verifyReset(
    { email: email.value, token: tokenString },
    {
      onSuccess: (data) => {
        if (data.verified) {
