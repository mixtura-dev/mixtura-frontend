    ...
    <div class="w-full mx-auto max-w-[1200px] my-8 px-4">
        ...
        <div class="flex justify-between my-8 flex-col gap-2 md:gap-3 md:flex-row">
            <InputGroup class="w-full bg-card md:w-64">
                <InputGroupInput type="search" v-model="search" placeholder="Search..." />
                <InputGroupAddon>
                    <Search />
                </InputGroupAddon>
        ...
        <template v-else-if="isEmptyResponse">
            <div class="space-y-4 bg-card rounded-lg border border-dashed p-6 text-center">
                <div class="space-y-1">
                    <p>No projects</p>
...
const isEmptyResponse = computed(() => !isLoading.value && (!workspaces.value || workspaces.value.length === 0));
