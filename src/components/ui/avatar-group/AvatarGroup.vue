...
<template>
    <div :class="cn('flex flex-row-reverse justify-end items-center', props.class)">
        <Avatar v-if="overflowCount > 0" class="ring-[0.125em] ring-background -me-1.5 first:me-0 z-[1]">
            <AvatarFallback class="text-[0.875em] bg-muted flex items-center justify-center">
                {{ props.overflowLabel ?? `+${overflowCount}` }}
            </AvatarFallback>
        </Avatar>

        <component v-for="(avatar, index) in displayedAvatars" :is="avatar" :key="index" v-bind="rootProps"
